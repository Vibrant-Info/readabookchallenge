	...
	request.on('end', function () {
		...
		if(ccavResponse != ""){
			...
			if( order_status == "Success" ){
				...
				htmlcode = '<html><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"><title>Response Handler</title></head><body><script>location.href = "http://readabookchallenge.com/#/profile";</script></body></html>';
				response.writeHeader(200, {"Content-Type": "text/html"});
				response.write(htmlcode);
				response.end(); 
			}
...
exports.postResFinal = function(request,response, connection){
	...
	request.on('end', function () {
		...
		if(ccavResponse != ""){
			...
			var type = arr[arr1.indexOf('payment_mode')];
			var ip_address = '';
