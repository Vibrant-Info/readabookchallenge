...
<?php foreach($list as $num_row => $row){ ?>        
		<tr  <?php if($num_row % 2 == 1){?>class="erow"<?php }?>>
			...
			<?php if($column->field_name != 'category_image'){ ?>
				<div class='text-left'><?php echo $row->{$column->field_name} != '' ? $row->{$column->field_name} : '&nbsp;' ; ?></div>
			<?php } else{ ?>
				<div class='text-left'><img src="<?php echo $row->{$column->field_name} != '' ? $row->{$column->field_name} : '&nbsp;' ; ?>" alt="catetgory" style="width: 95px;height: 82px;"></img></div>
			<?php } ?>
			</td>			
			<?php }?>
