...
<section class="content-header">
  <h1> <?php echo $this->lang->line("Assgining The Book"); ?> </h1>
  ...
  <div class="row">
    <div class="col-xs-12">
        <div class="grid_container" style="width:100%; height:700px;">
            ...
            url="<?php echo base_url()."librarian/assigning_list_data"; ?>" 
		...
		<div id="tb" style="padding:3px">
            <form class="form-inline" style="margin-top:20px"  enctype="multipart/form-data">
				...
				<div class="pull-right">
					<input type="button" class="btn btn-success" value="Send to package" id="bulkChange" />
				</div>
            </form>
...
var plan_id = 0;   
    ...
    {    
		...
		console.log(row.books_ids)
		if(row.order_count == null || row.order_count == ''|| row.order_count == undefined || row.order_count == 0 ){
			str += "<input type='button' class='btn btn-info btn-sm' value='select books' onclick='selectionProcess("+row.id+", "+number_of_books+")' >";
		}else{
			str += "<input type='button' class='btn btn-info btn-sm' value='Reselect books' onclick='reselectionProcess("+row.id+","+number_of_books+")' >";
			str += "&nbsp;&nbsp;&nbsp;&nbsp;<input type='button' class='btn btn-success btn-sm' data-books='"+row.books_ids+"' value='Ready to package' onclick='packageConfirmation("+row.id+", this)' >";
		}
	...
	var orderId, books_ids;
	function selectionProcess(order_id, books_count){
		$("#error").addClass('hide');
		orderId = order_id;
		$("#seletionPopup").modal();
		number_of_books = books_count;
		$("#order_books").html(books_count);
		$.ajax({
			url : base_url+'librarian/getBooks',
			data:{
				'orderId': orderId
	...
	function completeSelection(){
		var favorite = [];
		$.each($("input[name='book_id_checked']:checked"), function(){            
			favorite.push($(this).val());
		});
		if(number_of_books == favorite.length && favorite.length > 0){
			$.ajax({
				url : base_url+'librarian/OrderBooks',
				data:{
					'booksID': favorite.join(","),
					'orderId': orderId							
				},
				method:'POST',
				success:function(response){				
					$j('#tt').datagrid('load');
					$("#seletionPopup").modal('hide');
				}
			});
		}else{
	...
	function reselectionProcess(order_id, books_count){
		$("#error").addClass('hide');
		orderId = order_id;
		$("#seletionPopup").modal();
		number_of_books = books_count;
		$("#order_books").html(books_count);
		$.ajax({
			url : base_url+'librarian/editBooks',
			data:{
				'orderId': orderId								
			},
			method:'POST',
	...
	function packageConfirmation(order_id, ids){
		orderId = order_id;
		books_ids = $(ids).data('books');
		$("#confirmationPopup").modal();
	}
	function sendToPackage(){
		$.ajax({
			url : base_url+'librarian/sendToPackage',
			data:{
				'orderId': orderId,
				'booksID' :books_ids
			},
...
<div id="seletionPopup" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-header">
                ...
                <h4 id="new_search_details_title" class="modal-title"><i class="fa fa-cloud-upload"></i>Select Books</h4>
            </div><br/>
			<div class="modal-footer">
				<span class="pull-left">
					<p>Number of Books: <span id="order_books"></span></p>
				</span>
				<a class='btn btn-success' onclick="completeSelection()"> Done</a> 
			</div>
            <div class="modal-body">
				<p class="text-center hide" style="color:red" id="error">Selected book should be same as Number of books ordered</p>
...
<div id="confirmationPopup" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
				<form name="stockForm" method="POST" > 
					<p>Are you sure to move this order to Packaging?</p>
				</form>	
            </div> 
			<div class="modal-footer">
				<a class='btn btn-danger' data-dismiss="modal" > No</a> 
				<a class='btn btn-success' onclick="sendToPackage()"> Done</a> 
			</div>
        </div>
    </div>
</div>
