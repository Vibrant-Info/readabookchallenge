...
class admin extends Home
{
    ...
    */
    public function config_category()
    {
        ...
        $crud->order_by('category_name','asc');
        $crud->where('deleted', '0');
        $crud->set_subject($this->lang->line("genre"));
...
exit();*/
    ...
    */
    public function book_list_data()
    {
		...
		for($p=0;$p<count($info); $p++){
			if( $info[$p]['category_id'] != "" ){
				$category_query = "SELECT `category_name` FROM `category` WHERE `id` IN (".$info[$p]['category_id'].")";			
				...
				$info[$p]['category_name'] = $cate_name;
				$cate_name = "";
			}else{				
				$info[$p]['category_name'] = "";
			}
		}
    ...
    */
    public function update_book($id = 0)
    {
        ...
        $result3 = $this->basic->get_data($table3, $where3 = '', $select = '', $join = '', $limit = '', $start = null, $order_by = 'category_name ASC');
    ...
    */
    public function add_book()
    {
        ...
        $result2 = $this->basic->get_data($table2, $where='', $select='', $join='', $limit='', $start=null, $order_by='category_name');
    ...
    */
    public function add_book_action()
    {
        ...
        if ($_POST) {
            ...
            $this->form_validation->set_rules('notes',                 '<b>'.$this->lang->line("notes").'</b>',              'trim');
            $this->form_validation->set_rules('link',                 '<b>'.$this->lang->line("link").'</b>',                'trim');
            $this->form_validation->set_rules('cat',                 '<b>'.$this->lang->line("genre").'</b>',                  'trim');
            ...
            else
            {
                ...
                if ($temp) 
                {
					...
					for($p=0; $p< count($explode_category); $p++){
						$check_cat = $this->basic->execute_query("SELECT * FROM `category` WHERE `category_name` = '".trim(addslashes($explode_category[$p]))."'");
						echo "SELECT * FROM `category` WHERE `category_name` = '".trim(addslashes($explode_category[$p]))."'";
						if(count($check_cat) > 0){
							...
							$new_cat['category_name'] = trim($explode_category[$p]);
							$new_cat['deleted'] = '0';
    ...
    */
    public function requested_books_data()
    {
		...
		$where_simple['request_book.request_status'] = 'Pending';
        $where  = array('where'=>$where_simple);


        $offset = ($page-1)*$rows;
        $result = array();
        $select = array('request_book.id','users.first_name','request_book.book_title','request_book.isbn','request_book.author','request_book.request_date','request_book.edition','request_book.request_status');
    ...
    */
    public function dash_board()
    {
        ...
        $table_circulation = 'order_items';
		$where_circulation['is_return'] = '0';
        ...
        $data['total_issued'] = $this->basic->get_data('orders', $where=array('where'=>array('status'=>'closed')), $select=array('count(id) as total_issued'));
        $data['open'] = $this->basic->get_data('orders', $where=array('where'=>array('status'=>'open')), $select=array('count(id) as open'));
        $data['cancelled'] = $this->basic->get_data('orders', $where=array('where'=>array('status'=>'cancelled')), $select=array('count(id) as cancelled'));
        $data['returned'] = $this->basic->get_data('orders', $where=array('where'=>array('status'=>'returned')), $select=array('count(id) as returned'));
        $data['in_process'] = $this->basic->get_data('orders', $where=array('where'=>array('status'=>'selection','status'=>'packaging','status'=>'labeling')), $select=array('count(id) as in_process'));
    ...
    */
    public function barcode_generate()
    {
		...
		if($type != 'orders'){
			...
			$str = "";
			$str .= "<div class='' style=''>
					<div class='col-xs-12 book-label' style='padding: 0;'>
					<link href='".base_url()."bootstrap/css/bootstrap.min.css' rel='stylesheet' type='text/css' />
					...
					<script src='".base_url()."assets/js/jquery-barcode.min.js' />";
			$i = 0;
			//$str .= "";
			foreach ($info as $barcode) {
				...
				if(strlen($title_temp)<= 48){
					$title = $title_temp;
				}else{
					$title = substr($title_temp,0,48) . '...';
				}
				
				if(strlen($cate_name)<= 43){
					$cate_name = $cate_name;
				}else{
					$cate_name = substr($cate_name,0,43) . '...';
				}
				...
				$str .= "<div class='bar-xs' style='height:auto;'>
						<div class='col-xs-12 bar-holder' style='padding:0px'>
							<p style='margin-bottom: 4px; text-align: center; font-family: Kirvy-Bold; text-transform: uppercase; font-weight: 900; margin-top: 10px;'>Read A Book Challenge</p>
							<!--<img src='".$src."' style='width: 56%; height: 30%; margin: 0 auto 4px; display: block;' />	-->
							<div id='barcodeTarget".$i."' class='barcodeTarget' style='margin: auto;'></div>
						...
						<div class='col-xs-12' style='padding:0px 5px 0px 5px; position: relative;'>
							...
							<p style='margin-bottom: 0; font-family: Kirvy-Bold;'>".$vendor_id."-".$cate_name."</p>
						</div>
			...
			foreach ($info as $barcode) {
				...
				$check_previous_order_query = "SELECT id FROM orders WHERE (status = 'open' OR status = 'due') AND user_id = '".$barcode['user_id']."'";
				$check_previous_order = $this->basic->execute_query($check_previous_order_query);
				
				if( count($check_previous_order) > 0 ){
					$books_id_query = "SELECT order_id, GROUP_CONCAT(book_id) as books_ids FROM order_items where order_id = '".$check_previous_order[0]['id']."' GROUP BY order_id";
					$books_ids = $this->basic->execute_query($books_id_query);
					$pickup_books = $books_ids[0]['books_ids'];
				}else{					
					$pickup_books = "";
				}
				...
				$str .= "<div class='col-xs-12 bar-xs' style='min-height:192px; padding: 15px 0 10px 0;'>
					...
					<div class='col-xs-4 pin-cod-para' style='padding: 0;'>
						...
						<div><p class='book-id-holder'>".$pickup_books."</p></div>
					</div>
				</div>
    ...
    */
	...
	public function checkVendors(){
		if ($_SERVER['REQUEST_METHOD'] === 'GET') {
		 ...
		 if ($_POST) {
			 ...
			 if( $type == 'name')
				 $input['name'] = trim($_POST['value']);
			 if( $type == 'mail')
				 $input['email'] = trim($_POST['value']);
			 if( $type == 'phone')
				  $input['phone'] = trim($_POST['value']);
	...
	public function checkVendorUpdates(){
		if ($_SERVER['REQUEST_METHOD'] === 'GET') {
		 ...
		 if ($_POST) {
			 ...
			 if( $type == 'uniqid')
				 $input['unique_id'] = trim($_POST['value']);
			 if( $type == 'name')
				 $input['name'] = trim($_POST['value']);
			 if( $type == 'mail')
				 $input['email'] = trim($_POST['value']);
	...
	public function add_vendor_action()
    {
        ...
        if ($_POST) {
            ...
            $this->form_validation->set_rules('email',                 '<b>'.$this->lang->line("email").'</b>',               'trim');
 ...
 /*        $search_unique_id = $this->session->userdata('vendor_list_unique_id');
    ...
    */
	...
	public function updateUser(){
		//print_r($_REQUEST);
		$checkQuery = "SELECT * FROM users WHERE email_id = '".$_REQUEST['email_id']."' AND id != '".$_REQUEST['id']."'";
    ...
    */
	...
	public function checkCategories(){
		if ($_SERVER['REQUEST_METHOD'] === 'GET') {
		 ...
		 if ($_POST) {
			 $input['category_name'] = trim($_POST['value']);
    ...
    */
	public function update_bookstock_func(){
		...
		$query= "SELECT COUNT(`id`) as count FROM order_items WHERE book_id ='".$id."'";
		$count = $this->basic->execute_query($query);
		
		if( $count[0]['count'] == 1 ){
			$query= "UPDATE book_info SET number_of_books = number_of_books+1 WHERE id='".$id."'";
			$info = $this->basic->execute_complex_query($query);			
			
			$query= "UPDATE order_items SET is_return = 1 WHERE book_id='".$id."'";
			$info = $this->basic->execute_complex_query($query);			
    ...
    */
	public function list_Books(){
		...
		$query= "SELECT order_items.id, orders.order_no, users.first_name, users.last_name, users.email_id, users.phone_number, book_info.author, book_info.edition_year, book_info.publisher, book_info.title , book_info.isbn, book_info.publication_place, book_info.number_of_pages, book_info.number_of_books, book_info.dues FROM order_items, orders, users, book_info WHERE order_items.order_id=orders.id AND order_items.book_id=book_info.id AND orders.user_id=users.id AND book_info.deleted='0' AND order_items.is_return = 0";
		$info = $this->basic->execute_query($query);
		$res = array();
		if(count($info) > 0){
			/* for($i=0;$i < count($info);$i++ ){
				$res[$i]['id'] = $info[$i]['id'];
				$res[$i]['order_no'] = $info[$i]['order_no'];
				$res[$i]['first_name'] = $info[$i]['first_name'];
				$res[$i]['last_name'] = $info[$i]['last_name'];
				$res[$i]['email_id'] = $info[$i]['email_id'];
				$res[$i]['size1'] = $info[$i]['size1'];
				$res[$i]['price'] = $info[$i]['price'];
				$res[$i]['call_no'] = $info[$i]['call_no'];
				$res[$i]['location'] = $info[$i]['location'];
				$res[$i]['clue_page'] = $info[$i]['clue_page'];
				$res[$i]['editor'] = $info[$i]['editor'];
				$res[$i]['title'] = $info[$i]['title'];
				$res[$i]['edition'] = $info[$i]['edition'];
				$res[$i]['publishing_year'] = $info[$i]['publishing_year'];
				$res[$i]['publication_place'] = $info[$i]['publication_place'];
				$res[$i]['number_of_pages'] = $info[$i]['number_of_pages'];
				$res[$i]['number_of_books'] = $info[$i]['number_of_books'];
				$res[$i]['dues'] = $info[$i]['dues'];
				$res[$i]['isbn'] = $info[$i]['isbn'];
				$res[$i]['source_details'] = $info[$i]['source_details'];
				$res[$i]['notes'] = $info[$i]['notes'];
				$res[$i]['cover'] = $info[$i]['cover'];
				$res[$i]['add_date'] = $info[$i]['add_date'];
			} */
			$total_result = count($info);
			echo convert_to_grid_data($info, $total_result);
		}
		else{
			$res[0]= 'No Records Found';
		}
	}
